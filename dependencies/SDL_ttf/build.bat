...
if !COMPILE! equ 1 (
    echo INICIANDO LA COMPILACION DE SDL_ttf
    ...
    cmake -A %PLATFORM% -DSDL2_INCLUDE_DIR:PATH="%SDL2_INCLUDE_DIR%" -DSDL2_LIBRARY:FILEPATH="!SDL2_DEBUG_LIBRARY!" -DSDL2TTF_INSTALL:BOOL=0 %COMPILEDIR%
    msbuild "SDL2_ttf.sln" /p:configuration=Debug
    cmake -A %PLATFORM% -DSDL2_INCLUDE_DIR:PATH="%SDL2_INCLUDE_DIR%" -DSDL2_LIBRARY:FILEPATH="!SDL2_RELEASE_LIBRARY!" -DSDL2TTF_INSTALL:BOOL=0 %COMPILEDIR%
    msbuild "SDL2_ttf.sln" /p:configuration=Release
    cd .\Debug
    rename SDL2_ttf.dll SDL2_ttfd.dll
    rename SDL2_ttf.lib SDL2_ttfd.lib
    cd ..
    ...
    XCOPY /y /s .\Debug\SDL2_ttfd.dll %DLLFOLDERS%
    XCOPY /y /s .\Release\SDL2_ttf.dll %DLLFOLDERS%
    cd ..\..
    echo SDL_ttf compilado
) else (
    echo ULTIMA VERSION DE SDL_ttf DETECTADA, no es necesario compilar
)