...
void eden_render::RenderManager::InitManager(const std::string& appName)
{
	...
	CreateRoot(); // crea la ra�z
	Setup(); // y arranca la inicializaci�n base
...
{
	if (_root != nullptr) { // si ya no hay ra�z
		_root->saveConfig(); // guarda su configuraci�n
	}
...
void eden_render::RenderManager::CreateRoot()
{
	...
	if (!Ogre::FileSystemLayer::fileExists(pluginsPath)) { // si no existe el plugin -> excepci�n de Ogre
	...
	// crea una nueva ra�z en base a los plugins y la configuraci�n base de Ogre
...
{
	_root->showConfigDialog(nullptr); // desactiva el di�logo de configuraci�n de Ogre
	_root->initialise(false); // desactiva la inicializaci�n de la ra�z de Ogre
