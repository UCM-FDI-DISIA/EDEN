...
namespace eden_debug {
	...
	public:
		...
		EDEN_API DebugDrawer(std::string id, std::string sceneID);
		/// @brief Destructora publica de la clase
		EDEN_API ~DebugDrawer() override;

		/// @brief Elimina todas las lineas dibujadas hasta el momento
		EDEN_API void clearLines() override;
		...
		EDEN_API void drawLine(const btVector3& origin, const btVector3& end, const btVector3& color) override;
		...
		EDEN_API void drawRigidBody(btRigidBody* rb, const btVector3& color);
		...
		/// @param lifeTime 
		/// @param color 
		EDEN_API void drawContactPoint(const btVector3& PointOnB, const btVector3& normalOnB, btScalar distance, int lifeTime,
			const btVector3& color) override;

		/// @brief Necesario para override
		/// @param warningString Tipo de error recogido como cadena de caracteres
		EDEN_API void reportErrorWarning(const char* warningString) override;

		/// @brief Necesario para override
		/// @param location Lugar del mundo donde se quiere posicionar
		/// @param textString Texto que se quiere escribir
		EDEN_API void draw3dText(const btVector3& location, const char* textString) override;
